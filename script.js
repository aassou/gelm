...
//Societe section
function autocompletSociete() {
	var min_length = 1; // min caracters to display the autocomplete
	var keyword = $('#nomSociete').val();
	if (keyword.length >= min_length) {
		$.ajax({
			url: 'ajax-refresh-societe.php',
			type: 'POST',
			data: {keyword:keyword},
			success:function(data){
				$('#societeList').show();
				$('#societeList').html(data);
			}
		});
	} 
	else {
...
// set_item Projet : this function will be executed when we select an item
function setItemSociete(item1, item2){
	// change input value
	$('#nomSociete').val(item1);
	$('#idSociete').val(item2);
	// hide proposition list
	$('#societeList').hide();
}
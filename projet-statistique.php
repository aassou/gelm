<?php
    ...
    if(isset($_SESSION['userMerlaTrav']) and (isset($_GET['idProjet']))){
        ...
        ceil(
          ...
          +
          $terrainManager->getTotalPrixTerrainByIdProjet($idProjet)
          +
          $maisonManager->getTotalPrixMaisonByIdProjet($idProjet)
        );
        ...
        $sommeLivraisonsPayees = 0;
        $idsLivraisons = $livraisonsManager->getLivraisonIdsByIdProjet($idProjet);
        $idsLivraisonsNonPayees = $livraisonsManager->getLivraisonNonPayeesIdsByIdProjet($idProjet);
        $idsLivraisonsPayees = $livraisonsManager->getLivraisonPayeesIdsByIdProjet($idProjet);
        //calculate somme Livraisons total
        foreach ( $idsLivraisons as $id ) {
            $sommeLivraisons += $livraisonDetailManager->getTotalLivraisonByIdLivraison($id);
        }
        //calculate somme Livraisons Non Payees
        foreach ( $idsLivraisonsNonPayees as $id ) {
            $sommeLivraisonsNonPayees += $livraisonDetailManager->getTotalLivraisonByIdLivraison($id);
        }
        foreach ( $idsLivraisonsPayees as $id ) {
            $sommeLivraisonsPayees += $livraisonDetailManager->getTotalLivraisonByIdLivraison($id);
        }
        $sommeLivraisons = ceil($sommeLivraisons);
        $sommeLivraisonsNonPayees = ceil($sommeLivraisonsNonPayees);
        $sommeLivraisonsPayees = ceil($sommeLivraisonsPayees);
        //$sommeReglements = ceil($reglementsFournisseurManager->sommeReglementFournisseurByIdProjet($_GET['idProjet']));
        //$sommeLivraison = ceil($livraisonsManager->getTotalLivraisonsIdProjet($idProjet));
?>
<!DOCTYPE html>
        ...
        $(function() {
            ...
            $('#container2').highcharts({
                ...
                series: [
                {
                    ...
                    data: [<?= json_encode($sommeLivraisonsPayees) ?>]
                },
                {
                ...
                {
                    name: 'Reliquat',
                    data: [<?= json_encode($sommeLivraisonsNonPayees) ?>]
                }
                ]
            });
        });
